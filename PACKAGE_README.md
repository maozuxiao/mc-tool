...
以发布 `1.0.45` 为例（仓库根即应用根，路径都以 `mc-tool/` 为基准）：

```text
1. 打开 package.json
   把 "version": "1.0.44" 改为 "version": "1.0.45"

2. 打开 electron-builder.yml
   把 buildVersion: 1.0.44.0 改为 buildVersion: 1.0.45.0
...
electron-builder buildVersion: 1.0.44.0
```
...
按常规发布流程，把产品版本与文件版本统一进一位即可。注意 `package.json` 的 `version` 必须为合法 SemVer（三段），不能写成 `1.0.45.0`；文件版本 `buildVersion` 才是四位。

步骤：

```text
1. package.json         -> "version": "1.0.45"
2. electron-builder.yml -> buildVersion: 1.0.45.0
3. npm run pack:win
```
...
dist/
├── latest.yml
├── MC物料查询 Setup 1.0.45.exe
├── MC物料查询 Setup 1.0.45.exe.blockmap
└── MC物料查询 1.0.45.exe
```

发布时用 `.\build_now.ps1 -Publish` 把 `latest.yml` + 两个 exe + blockmap 上传为该版本的 Release 附件，
再把 draft 改为 **Published**；客户端（如 1.0.44）启动后即会检测到 `1.0.45` 并提示更新。
...
1. 升版本号：
   ...
   electron-builder.yml  -> buildVersion: 1.0.45.0
   ```
...
或直接 `.\build_now.ps1 -Publish` 后先别发布），已安装的旧版本（如 1.0.43/1.0.44）客户端即可检测到 1.0.45 升级；
