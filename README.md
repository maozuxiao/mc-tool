...
| 🖥 **OA 工作台（应用内）** | 点标题栏品牌区（图标或标题 / 倒计时）在应用内新窗口打开 OA 首页，复用当前登录会话、免扫码；窗口内各入口（新建流程、会议预约、我的工具等）`target="_blank"` 弹出的子窗口、以及跳转到 IAM / MC 等兄弟系统的页面**同样免登录**；按住 `Ctrl` / `⌘` 点击则改用系统默认浏览器打开。 |
...
| 打开 OA | 点击标题栏品牌区（图标 / 标题 / 倒计时均可）在应用内新窗口打开 OA 工作台（复用登录态、免扫码，窗口内的子窗口与外链同样免登录）；按住 `Ctrl` / `⌘` 点击改用系统浏览器。 |
