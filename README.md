...
| 📚 **鸿翼文件查询下载** | 在企业内容库（`wj.streamax.com`）里检索文件并**预览 / 下载原始文件**：下载走系统「另存为」并**边下边写**（显示百分比），预览在应用内窗口打开；首次使用自动弹出应用内登录窗口。 |
...
- 模型不会去本地磁盘「顺便搜一下」——这个技能**只在服务器侧检索**，结论只来自检索结果；如果没命中会告诉你换了哪些关键词，而不是断言「文件不存在」。
